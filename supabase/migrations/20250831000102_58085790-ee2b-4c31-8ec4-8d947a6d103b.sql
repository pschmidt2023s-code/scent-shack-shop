...
-- (Users who made guest orders will need to contact support to access their orders)
...
-- 5. Update insert policy to be more explicit
...
WITH CHECK (
  ...
  AND (
    ...
    OR
    -- For guest orders (only admins or edge functions can create these)
    (user_id IS NULL AND is_admin(auth.uid()) = true)
  )
);

-- Also secure the order_items table to prevent data leakage
...
-- Block anonymous access to order items
